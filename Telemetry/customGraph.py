...
class Graph(Widget):
    ...
    border_color = ListProperty([0, 1, 0, 0.2])
    ...
    def __init__(self, **kwargs):
        ...
        self.bind(center=ts, padding=ts, precisionx=ts, precisiony=ts, plots=ts, x_grid=ts,
    ...
    def _update_labels(self):
        ...
        ymax = self.ymax
        xmin = self.xmin
        precisionx = self.precisionx
        precisiony = self.precisiony
        x_overlap = False
        y_overlap = False
        ...
        if len(ylabels) and ylabel_grid:
            ...
            ylabels[0].text = precisiony % funcexp(ypoints[0])
            ...
            for k in range(len(ylabels)):
                ylabels[k].text = precisiony % funcexp(ypoints[k])
        ...
        if len(xlabels) and xlabel_grid:
            ...
            xlabels[0].text = precisionx % funcexp(xpoints[-1])
            ...
            if not x_next:
                xlabels[0].text = precisionx % funcexp(xpoints[0])
                xlabels[0].texture_update()
            ...
            for k in range(len(xlabels)):
                xlabels[k].text = precisionx % funcexp(xpoints[k])
    ...
    ymin1 = NumericProperty(0.)
    '''The y-axis minimum value.
    CUSTOM***********
    :data:`ymin` is a :class:`~kivy.properties.NumericProperty`, defaults to 0.
    '''

    ymax1 = NumericProperty(100.)
    '''The y-axis maximum value, larger than ymin.
    CUSTOM***********
    :data:`ymax` is a :class:`~kivy.properties.NumericProperty`, defaults to 0.
    '''

    ymin2 = NumericProperty(0.)
    '''The 2y-axis minimum value.
    CUSTOM***********
    :data:`ymin` is a :class:`~kivy.properties.NumericProperty`, defaults to 0.
    '''

    ymax2 = NumericProperty(100.)
    '''The 2y-axis maximum value, larger than ymin.
    CUSTOM***********
    :data:`ymax` is a :class:`~kivy.properties.NumericProperty`, defaults to 0.
    '''
    ...
    ylabel1 = StringProperty('')
    '''The label for the 1y-axis. If not empty it is displayed in the center of
    the axis.
    CUSTOM***********
    :data:`ylabel` is a :class:`~kivy.properties.StringProperty`,
    defaults to ''.
    ...
    ylabel2 = StringProperty('')
    '''The label for the 2y-axis. If not empty it is displayed in the center of
    the axis.
    CUSTOM***********
    :data:`ylabel` is a :class:`~kivy.properties.StringProperty`,
    defaults to ''.
