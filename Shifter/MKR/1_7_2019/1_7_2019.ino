...
#define spark_delay  5    //wait the ECU!
#define FULL        100
#define HALF        500
...
void canReads() {  
 ...
 /* if(id==1532) {       //rear right module
    rr=uint8_t(buf[0]);
  }
  else if(id==0x5fd) {  //rear left module
    rl=uint8_t(buf[0]);
    
  }
  else if(id==0x5fe) {  //front right module
    fr=uint8_t(buf[0]);
  }
  */
  if(id==0x600) {  //ECU
    rpm=uint16_t(buf[1]<<8 + buf[0]);
